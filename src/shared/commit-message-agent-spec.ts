...
export function resolveCommitMessageAgentChoice(
  ...
  if (
    ...
    if (getCommitMessageAgentSpec(defaultTuiAgent)) {
      return defaultTuiAgent
    }
    // Why: agents like opencode have no non-interactive commit-message spec;
    // fall through to the default rather than disabling the feature entirely.
  }
